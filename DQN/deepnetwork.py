...
class DeepQNetwork:
    ...
    def get_action(self, processed_state):
        ...
        if np.random.random() > self.epsilon:
            ...
            with tf.Session() as sess:
                ...
                _action = _action.eval(session = sess)
    ...
    def update_q_value(self, rewards, current_q_list, next_q_list, actions, done, discount_factor):
        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            current_action_qs = current_q_list.eval(session = sess)
            next_action_qs = next_q_list.eval(session = sess)
        sess.close()
    ...
    def train_step(self, states, actions):
        ...
        #print("loss",loss)
        gradients = tf.gradients(loss, self.train_network.trainable_variables)
