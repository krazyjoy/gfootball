...
class FootballEnv(gym.Env):
  ...
  def __init__(self, config):
    ...
    self._players = self._construct_players(config['players'], player_config) # get player class from commands
  ...
  def _construct_players(self, definitions, config):
    ...
    for definition in definitions:
      ...
      player = player_factory.Player(player_config, self._config) # 初始化一個player
      if name == 'agent':
